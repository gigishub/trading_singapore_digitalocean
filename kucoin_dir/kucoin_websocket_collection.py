...
class Kucoin_websocket_collection:
    ...
    async def initialize_websocket(self, symbol: str,ticker_channel : bool=None,level2_channel:bool =None ,match_level3_channel:bool=None ,best_ask_bid_channel: bool =None) -> bool:
        """
        ...
        try:
            ...
            if match_level3_channel:
                subscribe_data = orjson.dumps({
                    "id": int(time.time() * 1000),
                    "type": "subscribe",
                    "topic": f"/market/match:{symbol}-USDT",
                    "privateChannel": False,
                    "response": True
                }).decode('utf-8')
            
            if best_ask_bid_channel:
                subscribe_data = orjson.dumps({
                    "id": int(time.time() * 1000),
                    "type": "subscribe",
                    "topic": f"/spotMarket/level2Depth5:{symbol}-USDT",
                    "privateChannel": False,
                    "response": True
                }).decode('utf-8')
    ...
    async def get_price_websocket_ticker(self, symbol: str, max_wait_time: int = 2, release_time: datetime = None) -> Optional[float]:
        """
        Get price through WebSocket ticker channel.
        Args:
            symbol: Trading pair symbol
        ...
        success = await self.initialize_websocket(symbol, ticker_channel=True)
        if not success:
                    ...
                    if data.get('type') == 'message' and 'data' in data:
                        try:
                            ...
                            if price > 0:
                                ...
                                logger.debug(f"retrieved: {price}")
                    ...
                    if data.get('type') == 'message' and 'data' in data:
                        try:
                            ...
                            if data_from_socket:
                                ...
                                logger.debug(f"retrieved: {data_from_socket}")
                        except (KeyError, ValueError) as e:
                            logger.error(f"Failed to parse price: {e}")
                            continue
    ...
    async def get_price_websocket_match_level3(self, symbol: str, max_wait_time: int = 2, release_time: datetime = None) -> Optional[float]:
        """
        Get price through WebSocket level2 channel.
        Args:
            symbol: Trading pair symbol
        ...
        # Initialize WebSocket and wait for confirmation
        success = await self.initialize_websocket(symbol, match_level3_channel=True)
        if not success:
            logger.error("Failed to initialize WebSocket")
            return None
    ...
    async def get_price_websocket_best_ask_bid(self, symbol: str, max_wait_time: int = 2, release_time: datetime = None) -> Optional[float]:
        """
        Get price through WebSocket level2 channel.
        Args:
            symbol: Trading pair symbol
        ...
        # Initialize WebSocket and wait for confirmation
        success = await self.initialize_websocket(symbol, best_ask_bid_channel=True)
        if not success:
            logger.error("Failed to initialize WebSocket")
            return None
...
###########################################################################################
# testing with manual relase time 
###########################################################################################
...
async def main():
    ...
    if testing:
        ...
        release_date_time = datetime.now() + timedelta(seconds=3) 
...
##################################################################
# testing with dictonary and retrived time to activate turn testing to False

# add method after creating object to get price on relsease time 
###################################################################
        ...
        for filename in os.listdir(directory):
            ...
            if 0 < datetime_to_listing_seconds < 1200:
                ...
                try:
                    ...
                    # execution code here for testing 
