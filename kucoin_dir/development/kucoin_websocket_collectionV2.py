...
class KucoinWebsocketCollection:
    ...
    async def listen_to_updates(
        ...
        """
        Listen to WebSocket updates and print them.
            ...
            while True:
                try:
                    ...
                    if msg.type == aiohttp.WSMsgType.TEXT:
                        ...
                        if data.get('type') == 'message' and 'data' in data:
                            logger.debug(f"Data for {channel} channel received at {datetime.now().strftime('%H:%M:%S.%f')}: {data['data']}")
...
async def main():
    symbol = "BTC"
    channel_names = ['match'] #['ticker', 'level2', 'match', 'depth5']
    scrapers = []
    tasks = []
