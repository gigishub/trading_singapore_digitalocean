...
async def main():
    ...
    try:
        ...
        testing_time_offset = 6
        max_wait_time_for_execution = 10  # Time to wait for price
        
        num_buy_order_to_send = 2    # Number of buy orders to send with offset time
        time_offset_ms = 10
        # Define different parameter sets for percent_of_price_buy and percent_of_price_sell
        parameter_sets = [
            {'percent_of_price_buy': 1.2, 'percent_of_price_sell': 1},
            {'percent_of_price_buy': 1.1, 'percent_of_price_sell': 0.9},
            {'percent_of_price_buy': 1, 'percent_of_price_sell': 0.8}
                ...
                if filename.endswith(".json"):
                    ...
                    if 0 < datetime_to_listing_seconds < 1200:
                        ...
                        try:
                            ...
                            ws = KucoinWebsocketCollection()
                            ws_match_channel_response = await ws.get_price_at_release(
                                basecoin,
                                channel='match',
