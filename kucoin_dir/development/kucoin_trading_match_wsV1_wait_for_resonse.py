...
async def main():
    ...
    try:
        ...
        percent_of_price_sell = 0.4  # Setting limit order to sell n% of the actual buy price
                ...
                if filename.endswith(".json"):
                    ...
                    if 0 < datetime_to_listing_seconds < 1200:
                        ...
                        await execute_trading_strategy(
                            ...
                            percent_of_price_buy, percent_of_price_sell
                        )
...
async def execute_trading_strategy_multi(client, basecoin, release_date_time, max_wait_time_for_execution, percent_of_price_buy, percent_of_price_sell):
    ...
    try:
        ...
        logger.debug(f'Price retrieved: {websocket_release_price}')
        if websocket_release_price and datetime.now() < release_date_time + timedelta(seconds=max_wait_time_for_execution):
            try:
                ...
                # Create tasks to send multiple buy orders
                tasks = []
                for i in range(10):
                    tasks.append(
                        asyncio.create_task(
                    ...
                    logging.info(f'Buy order {i} placed')
                    await asyncio.sleep(0.2)
                # Wait for the first successful order response

                response,limit_order_buy_price= await asyncio.wait(tasks, return_when=asyncio.FIRST_COMPLETED)
                ...
                logger.error(f'Error in buying/selling execution: {e}')
                traceback.print_exc()
        else:
...
async def execute_trading_strategy(client, basecoin, release_date_time, max_wait_time_for_execution, percent_of_price_buy, percent_of_price_sell):
    """Execute the trading strategy."""
    try:
        scraper = Kucoin_websocket_collection()
        websocket_release_data = await scraper.get_price_websocket_match_level3(
